document.addEventListener("DOMContentLoaded", function () {
    ...
    window.editPlaylist = function(icon) {
        const playlistItem = icon.closest('.playlist-item');
        const playlistName = playlistItem.querySelector('span').textContent;
        const newName = prompt('Editar nome da playlist:', playlistName);
        if (newName && newName !== playlistName) {
            playlistItem.querySelector('span').textContent = newName;
        }
    };
    ...
    window.addPlaylist = function() {
        ...
        if (playlistName) {
            ...
            newPlaylistItem.innerHTML = `<span>${playlistName}</span><i class="fa-solid fa-pen" onclick="editPlaylist(this)"></i>`;
            playlistsList.insertBefore(newPlaylistItem, playlistsList.querySelector('.add-playlist'));
        }
    };

    window.deleteSong = function(icon) {
        const songItem = icon.closest('.song-item');
        songItem.remove();
    };

    window.addSong = function() {
        const songName = prompt('Nome da nova música:');
        if (songName) {
            const songsList = document.getElementById('songs-list');
            const newSongItem = document.createElement('li');
            newSongItem.classList.add('song-item');
            newSongItem.innerHTML = `<span>${songName}</span><i class="fa-solid fa-trash" onclick="deleteSong(this)"></i>`;
            songsList.insertBefore(newSongItem, songsList.querySelector('.add-song'));
        }
    };
});
