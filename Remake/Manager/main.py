    ...
    while True:
        ...
        if op == "1":
            ...
            while True:
                menu_win.clear()
                menu_win.border()
                menu_win.addstr(1, 2, "1 - Adicionar transmissão a canal")
                menu_win.addstr(2, 2, "2 - Informações de canal")
                menu_win.addstr(3, 2, "0 - Voltar")
                menu_win.refresh()

                op2 = get_input(menu_win, "Escolha uma opção:", 5, 2)
                if op2 == "1":
                    ip = get_input(menu_win, "IP do nó:", 7, 2)
                    zona_nome = get_input(menu_win, "Nome da zona:", 8, 2)
                    canal = get_input(menu_win, "Canal:", 9, 2)
                    msg = m.add_transmissao(ip, zona_nome, canal)
                elif op2 == "2":
                    canais = list(m.get_canais().keys())
                    msg_win.clear()
                    msg_win.border()
